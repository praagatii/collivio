...
export function ProjectCard({
  ...
  if (variant === "tile") {
    return (
      <motion.article
        ...
        className="group w-[min(84vw,340px)] shrink-0"
      >
        <Link href={href} onClick={go} data-cur className="block">
          <div className="relative block overflow-hidden border border-line aspect-[4/3] bg-tan">
            <img
              src={project.image}
              alt={project.title}
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            <div>
              ...
              <h3 className="disp mt-1.5 text-lg text-ink">{project.title}</h3>
            </div>
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-line text-ink transition-colors duration-300 group-hover:border-ink group-hover:bg-ink group-hover:text-canvas">
  ...
  return (
    ...
    >
      <Link href={href} onClick={go} data-cur className="block">
        <div className="grid gap-5 md:grid-cols-[minmax(0,2.1fr)_minmax(0,5fr)] md:gap-8">
          <div className="relative aspect-[16/10] overflow-hidden border border-line bg-tan">
            <img
              src={project.image}
              alt={project.title}
              loading={eager ? "eager" : "lazy"}
  ...
  if (variant === "tile") {
    return (
      <motion.article
        whileHover={{ y: -4 }}
        transition={{ type: "spring", stiffness: 300, damping: 24 }}
        className="group w-[min(84vw,340px)] shrink-0 border border-line bg-paper"
      >
        <Link href={href} onClick={go} data-cur className="block">
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            />
            <span className="absolute left-3 top-3 grid h-14 w-14 place-items-center rounded-full bg-ink text-canvas disp text-xl">
              {item.collaborators}
            </span>
          </div>
          <div className="p-5">
            <div className="label text-accent">{item.category}</div>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <ArrowUpRight size={17} className="text-ink transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
            </div>
          </div>
        </Link>
      </motion.article>
  ...
  return (
    ...
    >
      <Link href={href} onClick={go} data-cur className="block">
        ...
        >
          <div className="relative col-span-12 aspect-[16/9] md:col-span-5 md:aspect-auto">
            <img
              src={item.image}
              alt={item.title}
              loading="lazy"
...
export function ProfileCard({
  ...
  return (
    <motion.article
      whileHover={{ y: -4 }}
      transition={{ type: "spring", stiffness: 300, damping: 24 }}
      className={`group flex flex-col gap-4 border border-line bg-paper ${big ? "p-6" : "p-5"}`}
    >
...
export function MediaPost({
  ...
  return (
    <motion.article
      ...
      className={`group relative overflow-hidden border border-line bg-paper ${tall ? "aspect-[3/4]" : ""}`}
    >
